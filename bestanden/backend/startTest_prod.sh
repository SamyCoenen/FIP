...
# Jenkins heeft op dit moment de omgeving al gebuild. Naam docker image: groep7team/backend_prod
#verwijderen van vorige build, indien deze bestaat
docker rm -f backend_prod
...
docker run --name backend_prod groep7team/backend_prod /bin/bash -c  "python manage.py jenkins --enable-coverage --settings=fipsim.settings.production"
...
docker cp backend_prod:/code/. /var/jenkins_home/workspace/Test_Backend_Prod/
...
docker cp backend_prod:/code/production_env/reports/junit.xml /var/jenkins_home/workspace/backend_prod/
